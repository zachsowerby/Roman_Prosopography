...
combined_list = sorted(combined_list, key=lambda x: x[1], reverse=True)[0:6]
...
data = {}
data = Convert(pretty_list, data)

figures = list(data.keys())
percentages = list(data.values())

fig = plt.figure(figsize = (10, 5))

# creating the bar plot
plt.bar(figures, percentages, color ='blue', width = 0.4)

fig.autofmt_xdate()
plt.ylabel("Percent of Total Mentions")
